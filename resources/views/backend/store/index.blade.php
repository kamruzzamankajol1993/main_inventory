...
<div class="row">
    ...
    <div class="col-12">
        <h4 class="card-title">Store Listing</h4>
        <p class="card-title-desc">You can <code>Show, Hide & Edit</code> all Store</p>
    </div>
</div>
<!-- search bar --->
<div class="row">
...
<div class="row mt-2">
    ...
    <div class="col-sm-6">
        <div class="text-sm-end">
            <button type="button"
                ...
                <i class="mdi mdi-plus me-1"></i> Add New Store Info
            </button>
        </div>
            <!-- Modal -->
    ...
    <h5 class="modal-title" id="exampleModalLabel">Add Store Information</h5>
    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
    </button>
    </div>
    <div class="modal-body">
        <form class="mt-3" method="Post" action="{{ route('store_store') }}" enctype="multipart/form-data">
            @csrf
            <div class="row">
                ...
                <div class="table-responsive">
                    <table class="table  table-striped">
                        ...
                        <tbody>
                            ...
                            @foreach($store_infromation as $key=>$all_attribute)
                        <tr>
                            <td>
                                <div class="form-check font-size-16">
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    ...
                                    @if (Auth::guard('admin')->user()->can('store_update'))
...
<button class="btn btn-transparent btn-sm text-success" data-bs-toggle="modal" data-bs-target=".bs-example-modal-lg{{ $all_attribute->id }}">
    ...
    <!-- Modal -->
    @include('backend.store.edit')
@endif
    @if (Auth::guard('admin')->user()->can('store_delete'))
                                        ...
                                        <button class=" btn btn-transparent btn-sm text-danger"  onclick="deleteTag({{ $all_attribute->id}})"><i
                                                    ...
                                                    <form id="delete-form-{{ $all_attribute->id }}" action="{{ route('store_delete',$all_attribute->id) }}" method="POST" style="display: none;">
...
@endforeach
                ...
                @include('backend.store.normal_pagination')
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript" src="{{ asset('/') }}public/custom_js/store_page_search.js"></script>
<script type="text/javascript" src="{{ asset('/') }}public/custom_js/store_page.js"></script>
