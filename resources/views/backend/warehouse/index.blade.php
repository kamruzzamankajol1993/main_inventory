...
@section('title')
Warehouse information | {{ $ins_name }}
@endsection
...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
            <h4 class="mb-0">Warehouse Information</h4>
    ...
    <div class="col-12">
        ...
        <p class="card-title-desc">You can <code>Show, Hide & Edit</code> all Warehouse</p>
    </div>
</div>
<!-- search bar --->
<div class="row">
...
<div class="row mt-2">
    ...
    <div class="col-sm-6">
        <div class="text-sm-end">
            <button type="button"
                ...
                <i class="mdi mdi-plus me-1"></i> Add New Warehouse Info
            </button>
        </div>
            <!-- Modal -->
    ...
    <h5 class="modal-title" id="exampleModalLabel">Add Warehouse Information</h5>
    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
    </button>
    </div>
    <div class="modal-body">
        <form class="mt-3" method="Post" action="{{ route('ware_house_store') }}" enctype="multipart/form-data">
            @csrf
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="mb-3">
                        ...
                        <select name="incharge"  class="form-control" id="image">
                            ...
                            <option value="{{ $all_admin_list->name }}">{{ $all_admin_list->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table  table-striped">
                        ...
                        <tbody>
                            ...
                            @foreach($ware_house_infromation as $key=>$all_attribute)
                        <tr>
                            <td>
                                <div class="form-check font-size-16">
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    ...
                                    @if (Auth::guard('admin')->user()->can('ware_house_update'))
...
<button class="btn btn-transparent btn-sm text-success" data-bs-toggle="modal" data-bs-target=".bs-example-modal-lg{{ $all_attribute->id }}">
    ...
    <!-- Modal -->
    @include('backend.warehouse.edit')
@endif
    @if (Auth::guard('admin')->user()->can('ware_house_delete'))
                                        ...
                                        <button class=" btn btn-transparent btn-sm text-danger"  onclick="deleteTag({{ $all_attribute->id}})"><i
                                                    ...
                                                    <form id="delete-form-{{ $all_attribute->id }}" action="{{ route('ware_house_delete',$all_attribute->id) }}" method="POST" style="display: none;">
...
@endforeach
                ...
                @include('backend.warehouse.normal_pagination')
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript" src="{{ asset('/') }}public/custom_js/warehouse_page_search.js"></script>
<script type="text/javascript" src="{{ asset('/') }}public/custom_js/warehouse_page.js"></script>
